...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/BrightCloud05/Daat/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\daat" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\daat\hermes-agent" }),
...
$RepoUrlSsh = "git@github.com:BrightCloud05/Daat.git"
$RepoUrlHttps = "https://github.com/BrightCloud05/Daat.git"
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/BrightCloud05/Daat/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/BrightCloud05/Daat/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/BrightCloud05/Daat/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/BrightCloud05/Daat/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
