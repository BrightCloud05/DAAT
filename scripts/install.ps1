...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/BrightCloud05/BISEO/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\biseo" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\biseo\hermes-agent" }),
...
$RepoUrlSsh = "git@github.com:BrightCloud05/BISEO.git"
$RepoUrlHttps = "https://github.com/BrightCloud05/BISEO.git"
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/BrightCloud05/BISEO/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/BrightCloud05/BISEO/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/BrightCloud05/BISEO/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/BrightCloud05/BISEO/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
