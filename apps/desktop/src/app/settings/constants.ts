...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted BISEO & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when BISEO needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before BISEO stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters BISEO can read from one file request.',
  approvals: {
    mode: 'How BISEO handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When BISEO updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
