// "Hermes" kept alongside "Daat": the Python backend still emits the upstream
// wording on the wire (tui_gateway/server.py) until its display strings are swept.
const PROVIDER_SETUP_ERROR_RE =
  /No (?:inference|Hermes|Daat) provider(?: is)? configured|no_provider_configured|set an API key/i
