...
test('redactSecrets scrubs Authorization and X-BISEO-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-BISEO-Session-Token: hdr_888'), /X-Hermes-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-BISEO-Session-Token: hdr_888').includes('hdr_888'))
})
